...
public class BaseRazorPage : PageModel
{
    ...
    {
        if (context.HandlerMethod?.MethodInfo.Name != "OnPost") return;
    ...
    //public async Task<ContentResult> AjaxTryCatch(Func<Task<ApiResult>> func, bool isSuccessReloadPage = true, bool isErrorReloadPage = false, bool checkModelState = true)
    //{
    //    try
    //    {
    ...
    //public async Task<ContentResult> AjaxTryCatch<T>(Func<Task<ApiResult<T>>> func, bool isSuccessReloadPage = false, bool isErrorReloadPage = false)
    //{
    //    try
    //    {
